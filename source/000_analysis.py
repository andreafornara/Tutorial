...
# %%
'''
Just as an example we can plot the beta functions along the lattice:
'''
...
# %%
'''
Another crucial quantity is the closed orbit:
'''
...
# %%
'''
Other useful informations are stored in the twiss dataframe, for example the tune...
'''
...
# %%
'''
We want to change the optics of the line. To do this we need to use the knobs, which control the lens of the line.

We can have a configuration file with the knob settings, or we can set them manually.

Let's start with the manual setting of a knob: we change the (half) crossing angle at IP8 to 170 urad.

The steps are:
- Change the knob;
- Check that the knob has been changed;
- Perform the twiss again;
- Plot the closed orbit;

Then we can calculate the crossing angle at ip8 and compare it with the knob value to see if everything is ok.
'''
...
# %%
'''
We retrieve the expected value!

Now we want to change the knobs from a config file.
'''
