...
for ii in range(len(x)):
    qx_i.append(pnf.naff(x[ii,:], turns=n_turns, nterms=1, skipTurns=0, getFullSpectrum=False, window=1)[0][1])
    qy_i.append(pnf.naff(y[ii,:], turns=n_turns, nterms=1, skipTurns=0, getFullSpectrum=False, window=1)[0][1])
qx_i = np.array(qx_i)
qy_i = np.array(qy_i)
# plot the tunes
